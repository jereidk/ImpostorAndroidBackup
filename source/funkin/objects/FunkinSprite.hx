...
class FunkinSprite extends FlxAnimate
{
	...
	public function loadAtlas(path:String, ?settings:FlxAnimateSettings, mode:PathsTestMode = NORMAL):FunkinSprite
	{
		...
		for (path in splitPath)
		{
			...
			final isAtlasSprite = FunkinAssets.exists(Paths.getPath('images/$path/Animation.json', null, mode));
			if (isAtlasSprite)
			{
				var atlas = FlxAnimateFrames.fromAnimate(Paths.getPath('images/$path', null, mode), null, null, null, false, settings ?? {cacheOnLoad: true});
				if (atlas != null)
				{
