...
class WeekData
{
	...
	static function addSection(weekFile:WeekData):Void
	{
		...
		var path:String = Paths.getPath('weeks/freeplay/$sec.json', null, NORMAL), section:FreeplaySectionFile = null;
		
		if (!FunkinAssets.exists(path)) path = Paths.getPath('data/weeks/freeplay/$sec.json', null, NORMAL);
